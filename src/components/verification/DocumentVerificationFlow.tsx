...
const DocumentaryIdentityStep: React.FC<{
  ...
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
    <div className="space-y-3 text-center">
      ...
      <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Verificación documental premium</h1>
      <p className="font-medium text-slate-500">Sumá DNI y selfie como una capa opcional para dar más contexto a tu perfil.</p>
    </div>
      ...
      <div className="rounded-[24px] border border-amber-200 bg-amber-50 p-5 dark:border-amber-900/30 dark:bg-amber-900/20">
        <div className="flex gap-3">
          ...
          <p className="text-xs font-medium leading-relaxed text-amber-900 dark:text-amber-200">
            Si llegaste directo a esta pantalla, primero activá la mejora premium desde tu perfil para continuar sin errores.
          </p>
        </div>
      </div>
    ) : null}
    ...
    <div className="rounded-[24px] border border-amber-100 bg-amber-50 p-5 dark:border-amber-900/30 dark:bg-amber-900/20">
      <div className="flex gap-3">
        ...
        <p className="text-xs font-medium leading-relaxed text-amber-900 dark:text-amber-200">
          Este refuerzo no reemplaza el nivel principal de tu cuenta. Los primeros niveles siguen construyéndose con email, teléfono, perfil, actividad e historial real.
        </p>
      </div>
    </div>
  </motion.div>
...
const DocumentaryReadyStep: React.FC = () => (
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
    <div className="space-y-3 text-center">
      ...
      <p className="font-medium text-slate-500">Ya cargaste la base documental. El último paso deja activa esta capa premium en tu cuenta.</p>
    </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-5 rounded-[24px] border border-emerald-100 bg-emerald-50 p-6 dark:border-emerald-900/30 dark:bg-emerald-900/10">
        ...
        <div>
          ...
          <p className="text-xs font-medium text-emerald-600/70">Ya podés confirmar la mejora documental.</p>
        </div>
      </div>
    </div>
  </motion.div>
...
const OnsiteSuccessStep: React.FC<{ appointmentDate: string; propertyTitle?: string | null }> = ({ appointmentDate, propertyTitle }) => (
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
    <div className="space-y-3 text-center">
      ...
      <p className="font-medium text-slate-500">
        {propertyTitle ? `La mejora premium de ${propertyTitle} ya quedó activa.` : 'La mejora premium ya quedó activa para esta publicación.'}
      </p>
    </div>
...
export const DocumentVerificationFlow: React.FC<DocumentVerificationFlowProps> = ({
  ...
  const primaryButtonLabel = useMemo(() => {
    ...
    return step === 3 ? 'Finalizar verificación' : 'Continuar';
  }, [mode, step]);
  ...
  const handleDocumentaryNext = async () => {
    ...
    try {
      ...
      showToast('Verificación', error instanceof Error ? error.message : 'No pudimos completar este refuerzo documental.', 'error');
    } finally {
      setIsVerifying(false);
    }
  };
