...
export const GuestRequestProfileCard: React.FC<GuestRequestProfileCardProps> = ({ guestName, profile, className }) => {
  const historyItems = [
    ...
    ...(profile.platformHistory.conflictsCount > 0 ? [{ label: 'Conflictos', value: profile.platformHistory.conflictsCount }] : []),
    ...(profile.platformHistory.cancellationsCount > 0 ? [{ label: 'Cancelaciones', value: profile.platformHistory.cancellationsCount }] : []),
  ];
