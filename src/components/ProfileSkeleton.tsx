      ...
      <main className="app-page py-8 md:py-10">
        <div className="grid gap-6 xl:grid-cols-[320px_minmax(0,1fr)] xl:items-start">
          <aside className="space-y-6">
            <div className="bg-white dark:bg-slate-900 rounded-[32px] p-6 border border-slate-200 dark:border-slate-800 space-y-5">
              <div className="flex flex-col items-center space-y-4">
                <div className="h-28 w-28 rounded-full bg-slate-200 dark:bg-slate-800 skeleton" />
                <div className="space-y-2 w-full max-w-[220px]">
                  <div className="h-6 w-36 mx-auto rounded-lg skeleton bg-slate-200 dark:bg-slate-800" />
                  <div className="h-4 w-28 mx-auto rounded skeleton bg-slate-200 dark:bg-slate-800" />
                </div>
                <div className="flex gap-2">
                  <div className="h-8 w-24 rounded-full skeleton bg-slate-200 dark:bg-slate-800" />
                  <div className="h-8 w-24 rounded-full skeleton bg-slate-200 dark:bg-slate-800" />
                </div>
              </div>

              {[1, 2].map((i) => (
                <div key={i} className="rounded-2xl bg-slate-50 dark:bg-slate-800/50 p-4 space-y-2">
                  <div className="h-3 w-20 rounded skeleton bg-slate-200 dark:bg-slate-700" />
                  <div className="h-4 w-40 rounded skeleton bg-slate-200 dark:bg-slate-700" />
                </div>
              ))}
            </div>

            <div className="bg-white dark:bg-slate-900 rounded-[32px] p-6 border border-slate-200 dark:border-slate-800 space-y-4">
              <div className="h-5 w-36 rounded-lg skeleton bg-slate-200 dark:bg-slate-800" />
              {[1, 2, 3, 4].map((i) => (
