...
export const PropertyCard: React.FC<PropertyCardProps> = ({ 
  ...
  const hostExperienceYears = Number(property.hostExperienceYears || 0);
  ...
  const heroBadges = [
    property.isVerifiedProperty
      ...
      : property.identityValidated
        ? {
            ...
            className: 'border-white/70 bg-white/92 text-slate-700 shadow-sm',
          }
        : null,
    property.isSuperHost
      ? {
          ...
          className: 'border-white/70 bg-emerald-50/95 text-emerald-700 shadow-sm',
        }
      : !property.isVerifiedProperty && property.locationVerified
        ? {
            key: 'location',
            label: 'Ubicación verificada',
            variant: 'info' as const,
            icon: <Icons.MapPin className="h-3.5 w-3.5" />,
            className: 'border-white/70 bg-sky-50/95 text-sky-700 shadow-sm',
          }
        : null,
  ...
  const supportBadge = hostExperienceYears > 0
    ? {
        label: `${hostExperienceYears} ${hostExperienceYears === 1 ? 'año' : 'años'} en la plataforma`,
        variant: 'neutral' as const,
        icon: <Icons.Clock className="h-3.5 w-3.5 text-slate-400" />,
    ...
    : property.traceabilityLevel === 'high'
      ...
      : property.traceabilityLevel === 'medium'
        ? {
    ...
    >
        ...
        <div className="absolute inset-x-0 top-0 flex items-start justify-between gap-3 p-4">
          <div className="flex min-w-0 flex-wrap gap-2">
            {heroBadges.map((badge) => (
              <Badge key={badge.key} variant={badge.variant} size="md" className={badge.className}>
                {badge.icon}
                {badge.label}
              </Badge>
            ))}
      ...
      <div className="flex flex-1 flex-col gap-4 p-4 sm:p-5 md:p-6">
          ...
          <div className="min-w-0 flex-1 space-y-2">
            ...
            {property.hostName ? (
              <p className="truncate text-xs font-medium uppercase tracking-[0.14em] text-slate-400">
                Anfitrión <span className="text-slate-500">{property.hostName}</span>
              </p>
            ) : null}
          </div>
          <div className="shrink-0 rounded-full border border-slate-200/80 bg-white px-3 py-2 text-right shadow-[var(--app-shadow-subtle)]">
        ...
        <div className="flex flex-wrap items-center gap-2.5 text-sm text-slate-500">
          <Badge variant="neutral" size="md" className="border-slate-200 bg-slate-50 text-slate-700">
            <Icons.Users className="h-3.5 w-3.5 text-slate-400" />
            {property.maxGuests || 4} huéspedes
          </Badge>
          {supportBadge ? (
            <Badge variant={supportBadge.variant} size="md" className={supportBadge.className}>
              {supportBadge.icon}
              {supportBadge.label}
            </Badge>
          ) : null}
        ...
        <div className="mt-auto flex items-end justify-between gap-4 border-t border-slate-100/90 pt-4">
          <div className="space-y-1">
            <span className="block text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-400">Por noche</span>
